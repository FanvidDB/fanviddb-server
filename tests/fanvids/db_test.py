...
from fanviddb.fanvids.db import filename_to_tsquery
...
def test_filename_to_tsquery__basic():
    ...
    assert params["to_tsquery_1"] == "english"
    assert params["to_tsquery_2"] == "Hello"
...
def test_filename_to_tsquery__removes(punctuation):
    ...
    params = tsquery.compile().params
    assert params["to_tsquery_1"] == "english"
    assert params["to_tsquery_2"] == "Fandom | Creator | Title"
